...
public class UsingGitHubAPI {
    ...
    public void buildPRModel(String ghToken, String repositoryName) {
        try {
            ...
            prSearch.isClosed();
            LocalDate from = LocalDate.parse("2024-04-01");
            LocalDate to = LocalDate.parse("2024-04-03");
    ...
    private void execute() {
        for (GHPullRequest pullRequest : pullRequests) {
            ...
            System.out.println("firstHumanResponse " + firstHumanResponse(pullRequest));
    ...
    public long firstHumanResponse(GHPullRequest pullRequest)throws IOException{
    	 for (GHIssueEvent evi : pullRequest.listEvents()) {
             if (evi.getActor().getId() != pullRequest.getUser().getId()) {
                long timeDiff = evi.getCreatedAt().getTime() - pullRequest.getCreatedAt().getTime();
                return TimeUnit.MILLISECONDS.toMinutes(timeDiff);
             }
         }
    	return -1;
